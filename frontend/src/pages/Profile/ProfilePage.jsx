...
const ProfilePage = () => {
  ...
  // Sync user data to formData and avatar
  useEffect(() => {
    if (user) {
      setFormData({
        name: user.name || "",
  ...
  // Fetch Orders, Addresses, and Payments
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [ordersRes, addrRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/orders/user`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/addresses`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/auth/avatar`, data, {
  ...
  const handleProfileUpdate = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/auth/update`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveAddress = async (addr) => {
    try {
      const token = localStorage.getItem("token");
      if (addr._id) {
        await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/addresses/${addr._id}`, addr, {
          headers: { Authorization: `Bearer ${token}` },
        });
      } else {
    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8 bg-white p-6 rounded shadow">
        <div className="relative">
          <img
            src={avatar || `https://api.dicebear.com/9.x/initials/svg?seed=${user.name || "User"}`}
            alt="Profile"
  ...
  Logout
      ...
      <div className="bg-white p-6 rounded shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses
  .filter(addr => addr) // remove undefined/null
  .map((addr, idx) => (
